...
public class TechniqueEvaluationPlugin(ILogger<TechniqueEvaluationPlugin> logger)
{
        ...
        [Description("Player skill level for context")] string playerLevel = "Intermediate")
    {
        ...
        try
        {
            ...
            logger.LogInformation(
                "[TechniqueEvaluation] LLM response received in {ElapsedMs}ms. ResponseLength={ResponseLength}",
        ...
        catch (Exception ex)
        {
            sw.Stop();
            logger.LogError(ex,
                "[TechniqueEvaluation] LLM call failed after {ElapsedMs}ms. PlayerLevel={PlayerLevel}",
