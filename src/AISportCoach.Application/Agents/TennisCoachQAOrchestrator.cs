...
public class TennisCoachQAOrchestrator(Kernel kernel, ILogger<TennisCoachQAOrchestrator> logger)
{
    ...
    {
        logger.LogInformation("[TennisCoachQA] Processing question. Length={Length}", question.Length);
        ...
        var neededAgentNames = await TennisCoachQAManager.ClassifyAsync(chatService, question, allAgentNames, cancellationToken);

        logger.LogInformation("[TennisCoachQA] Classified to agents: {Agents}", string.Join(", ", neededAgentNames));

        var manager = new TennisCoachQAManager(neededAgentNames);
        ...
        logger.LogDebug("[TennisCoachQA] Raw GroupChat response. Length={Length}", rawJson?.Length ?? 0);
    ...
    private static CoachAnswerResult Merge(string? rawJson, ILogger logger)
    {
        ...
        {
            logger.LogWarning(ex, "[TennisCoachQA] Failed to parse merged response. Raw={Raw}",
...
internal sealed class TennisCoachQAManager : RoundRobinGroupChatManager
{
    private readonly IReadOnlyList<string> _neededAgentNames;
    private readonly Queue<string> _pendingAgents;

    public TennisCoachQAManager(IReadOnlyList<string> neededAgentNames)
    {
